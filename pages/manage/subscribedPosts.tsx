...
const ManageSubscribedPosts = () => {
  ...
  const { data: userSubscribedPosts, isFetching: isFetchingPosts } =
    useGetUserSubscribedPosts();
  const { isFetching: isFetchingSearch } = useSearchUserSubscribedPosts(
    String(searchKeyword),
    String(searchType)
  );
  const { data: category, isFetching: isFetchingSearchCategory } =
    useSearchCategoryOnUserSubscribedPosts(Number(categoryId));

  const [manageSubscribedPosts, setManageSubscribedPosts] = useRecoilState(
  ...
  return (
    <ManageLayout>
      <Head>
        ...
        {searchKeyword && (
          <title>Groom | 구독 글 관리 '{searchKeyword}'의 검색결과</title>
        )}
        {categoryId && (
          <title>
            Groom | 구독 글 관리 '{category?.name}' 카테고리의 글 목록
          </title>
        )}
      </Head>
